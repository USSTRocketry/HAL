...
std::pair<Wq::Status, Wh> Wq::Submit(const SubmitOptions& Options)
{
    ...
    auto* pTask = new Task(static_cast<unsigned long>(Options.Sched.DelayMs),
                           IterationsCount,
                           nullptr,
                           &TargetScheduler,
                           true,
    ...
    // consider fix
    auto HandleImpl = std::make_shared<Wh::Impl>(pTask, Options.Exec.Ctx);
    Wh Handle {HandleImpl};

    // Callback captures a copy of WorkHandle safely
    pTask->setCallback([Fn = Options.Exec.Fn, Handle]() mutable -> void { Fn(Handle); });
    pTask->setOnDisable(
